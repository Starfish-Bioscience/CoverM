...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        *)
            ...
            echo "Usage: $0 [--install] [--push] [--dry-run] [--force]"
            exit 1
            ;;
    esac
done
...
if [ "$DRY_RUN" = true ]; then
    ...
    if [ -f "$SIF_FILE" ] && [ "$FORCE" = false ]; then
        echo "  Build     : skip (SIF exists — use --force to rebuild)"
    else
...
# Skip build if SIF already exists (unless --force)
if [ -f "$SIF_FILE" ] && [ "$FORCE" = false ]; then
