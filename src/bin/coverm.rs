...
struct EstimatorsAndTaker {
    ...
    /// Extra column headers generated by --regions-bed (NB_LABELS × NB_METHODS)
    label_extra_headers: Vec<String>,
}
...
impl EstimatorsAndTaker {
    pub fn generate_from_clap(m: &clap::ArgMatches, stream: OutputWriter) -> EstimatorsAndTaker {
        ...
        EstimatorsAndTaker {
            ...
            tpm_column,
            printer,
            label_extra_headers: vec![],
        }
    }

    /// Register NB_LABELS × NB_METHODS extra columns generated by --regions-bed.
    ///
    /// Must be called BEFORE `print_headers`.  Updates `num_coverages` in
    /// `CachedSingleFloatCoverageTaker` so that the taker accepts the additional
    /// `add_single_coverage` calls emitted by `take_label_coverages`.
    pub fn extend_label_columns(&mut self, labels: &[String], label_methods: &[CoverageEstimator]) {
        if label_methods.is_empty() || labels.is_empty() {
            return;
        }
        let mut extra: usize = 0;
        for label in labels {
            for method in label_methods {
                // Invariant: every estimator allowed by make_label_estimators
                // contributes exactly one column_header and one add_single_coverage
                // call per genome.  assert in debug builds so divergence is caught
                // early if new estimator variants are introduced.
                debug_assert_eq!(
                    method.column_headers().len(),
                    1,
                    "estimator {:?} produces {} headers; expected 1",
                    method,
                    method.column_headers().len()
                );
                for header in method.column_headers() {
                    self.label_extra_headers
                        .push(format!("{} {}", header, label));
                    extra += 1;
                }
            }
        }
        // Update num_coverages so CachedSingleFloatCoverageTaker accepts the
        // extra add_single_coverage calls per entry.
        // NOTE: If a new CoverageTakerType variant is introduced that also tracks
        // a coverage count, it must be handled here too.
        if let CoverageTakerType::CachedSingleFloatCoverageTaker {
            ref mut num_coverages,
            ..
        } = self.taker
        {
            *num_coverages += extra;
    ...
    pub fn print_headers(mut self, entry_type: &str, print_stream: OutputWriter) -> Self {
        ...
        // Append per-label column headers (if any)
        headers.extend(self.label_extra_headers.iter().cloned());
        self.printer
